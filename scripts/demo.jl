...
drugs = [
    ...
    DrugRecord(;
        name="experimental_gaba", targets=["GABA_R"], evidence_tier=REPURPOSING_HYPOTHESIS
    ),
]
cands, warning = rank_treatments(["SCN1A"], drugs, net; n_bootstrap=30, rng=Xoshiro(0))
println("\n[4] Treatment ranking for disease module {SCN1A}:")
for (i, c) in enumerate(cands)
    println(
        "    $(i). $(c.drug.name)  tier=$(c.drug.evidence_tier)  ",
